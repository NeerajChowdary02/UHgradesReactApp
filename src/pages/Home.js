...
function Home() {
    ...
    useEffect(async () => {
        const res = await fetch('https://uhgrades-api.herokuapp.com/search/?search=Rizk')
    ...
    const searchCountry = async term => {
        if(term.length < 3 || term === '') {
            ...
            const res = await fetch(`https://uhgrades-api.herokuapp.com/search/?search=${term}`)
            const data = await res.json()
        ...
        <div className="bg-gray-100 dark:bg-gray-800 dark:text-white">
            <div className="w-screen shadow-md py-6 px-3 bg-white dark:bg-gray-700 dark:text-white mb-16">
                <div className="flex container mx-auto">
                    <h1 className="font-bold text-xl">UH Grades by Neeraj Chowdary Mamillapalli</h1>
